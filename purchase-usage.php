...
<?php echo $static->sidebar("inv-purchase-usage");?>
    ...
    <div id="main-content">
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-12">
                    <!--Top header start-->
                    <h3 class="ls-top-header"><i class='fa fa-codepen'></i> Inventory <i class="fa fa-angle-double-right"></i>  Purchase <i class="fa fa-angle-double-right"></i> Usage</h3>
                    <!--Top header end -->
					...
					<?php
                     ...
                     <?php 
								if(isset($val_company)){
									$q = mysql_query(
									"
										select pu.*,ip.name from purchase_usage pu
										left join purchase_stock ps on ps.id = pu.stock_item 
										left join item_purchase ip on ip.id = ps.item
										where (pu.company='{$val_company['id']}' and pu.created_by ='{$_SESSION['clerk']['id']}')
									");
									...
									"
										...
										left join purchase_stock ps on ps.id = pu.stock_item 
										left join item_purchase ip on ip.id = ps.item
										where (pu.company='{$arr[0]['id']}' and pu.created_by ='{$_SESSION['clerk']['id']}')
									");
									$prefix = $arr[0]['prefix'];
								}
