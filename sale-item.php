...
<?php echo $static->sidebar("inv-sale-item");?>
					...
					<?php
                     ...
                     <?php 
								if(isset($val_company)){
									$q = mysql_query("select * from item where company='{$val_company['id']}' and created_by='{$_SESSION['clerk']['id']}' and status=true order by quantity ASC");
									$prefix = $val_company['prefix'];
								}else{
									$q = mysql_query("select * from item where company='{$arr[0]['id']}' and created_by='{$_SESSION['clerk']['id']}' and status=true order by quantity ASC");
									$prefix = $arr[0]['prefix'];
								}
															...
															<tbody>
																...
																<?php 
																	...
																	while($result = mysql_fetch_assoc($q)){
																		...
																		$result['quantity']<=$settings['item_threshold'] ? $bg = " bg-danger" : $bg=NULL;
																		...
																		<tr>
																			...
																			<td class="text-center<?php echo $bg; ?>"><?php echo $result['name'];?></td>
																			<td class="text-center<?php echo $bg; ?>"><?php echo $result['quantity'];?></td>
																			<td class="text-center<?php echo $bg; ?>"><?php echo $result['volume'];?></td>
																			<td class="text-center<?php echo $bg; ?>"><?php echo $member->ret_by("unit","id",$result['unit'],"name");?></td>
																			<td class="text-center<?php echo $bg; ?>"><?php echo $result['price_per_unit'];?></td>
																			<td class="text-center<?php echo $bg; ?>">
																				<?php echo ($result['sale_item_desc']!=NULL ? $result['sale_item_desc'] : "-");?>
																			</td>
																			<td class="text-center<?php echo $bg; ?>"><?php echo date('d, M Y',strtotime($result['reg_date']));?></td>
																			<td class='text-center<?php echo $bg; ?>'>
																				<button  data-toggle="modal" data-target="#transfer_item" class="btn btn-xs btn-round btn-primary" onclick=retrieve({'act':'item_transfer_warehouse','item':'<?php echo $result['id']?>','qty':'<?php echo $result['quantity']?>'})><i class="fa fa-mail-forward"></i></button>
																				<button  data-toggle="modal" data-target="#add_quantity" class="btn btn-xs btn-round btn-primary" onclick=retrieve({'act':'add_sale_item_qty','item':'<?php echo $result['id']?>','qty':'<?php echo $result['quantity']?>'})><i class="fa fa-plus"></i></button>
																				<button type="button" class="btn btn-xs btn-round btn-primary" onclick=retrieve({'act':'edit_sale_item_qty','item':'<?php echo $result['id']?>'})><i class="fa fa-pencil"></i></button>
																				<button class="btn btn-xs btn-round btn-danger" data-toggle="modal" data-target="#dell_item" onclick=$("#remove_item").attr("onclick","retrieve({'act':'rem_sale_item','item':'<?php echo $result['id'];?>'})")><i class="fa fa-times"></i></button>
																			</td>
																		</tr>
																		<?php
									...
									<span id="item_create_msgbox"></span>
										<div class='col-md-12'>
													...
													<div class="form-group">
														...
														<textarea id="sale_item_desc" class="form-control" placeholder="Enter Description"></textarea>
													</div>
                    ...
                    <div class='row'>
										...
										<div class='col-md-12'>
											<span id="edit_notify"></span>		
													<div class="form-group">
														<label>Name*</label>
														<input type="text"  id="edit_curr_name" class="form-control" placeholder="Enter Name"/>
													</div>
													<div class="form-group">
														<label>Quantity*</label>
													...
													<div class="row">
														<div class="form-group col-md-6">
															<label>Volume/Item*</label>
															<input type="number" id="edit_curr_volume" class="form-control" placeholder="Enter Volume"/>
														</div>
														<div id="edit_curr_unit_container" class="form-group col-md-6"></div>
													</div>
													<div class="form-group">
														<label>Price Per Unit*</label>
														<input type="number" id="edit_curr_price_per_unit" class="form-control" placeholder="Enter Price Per Unit"/>
													</div>
													<div class="form-group">
														<label>Description*</label>
														<textarea id="edit_curr_desc" class="form-control" placeholder="Enter Description"></textarea>
													</div>
													<div class="form-group">
														<label>Action</label>
