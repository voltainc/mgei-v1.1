...
if(isset($_POST['remove_grouping']))
	{
		$remove_grouping = trim($_POST['remove_grouping']);
		if(!empty($remove_grouping))
		{
			mysql_query("update grouping set status=false where id={$remove_grouping}");
			$msgbox = "<div class='alert alert-success'>
			<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>×</button>
			<strong><i class='fa fa-check-circle'></i> Success!</strong> Grouping Removed
			</div>";
		}
	}
?>
...
<?php echo $static->sidebar("inv-grouping");?>
...
<section id="min-wrapper">
    <div id="main-content">
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-12">
                    <!--Top header start-->
                    <h3 class="ls-top-header"><i class='fa fa-codepen'></i> Inventory <i class="fa fa-angle-double-right"></i> Grouping</h3>
                    <!--Top header end -->
            ...
            <!-- Main Content Element  Start-->
					  ...
					  <div class="row">
					  <div data-toggle="modal" data-target="#add" class="col-md-3 col-sm-6">
                            <div class="ls-wizard label-red margin-top-iPad-15">
                                <h2>Grouping <small>(<?php echo $member->total_grouping($_SESSION['clerk']['id']);?>) Total</small></h2>
                                <span style="font-size:23px;">Add</span>
                                <div class="icon">
					...
					<?php
                     ...
                     <?php 
								if(isset($val_company)){
									$q = mysql_query("select * from grouping where company='{$val_company['id']}' and created_by='{$_SESSION['clerk']['id']}' and status=true order by id DESC");
									$prefix = $val_company['prefix'];
								}else{
									$q = mysql_query("select * from grouping where company='{$arr[0]['id']}' and created_by='{$_SESSION['clerk']['id']}' and status=true order by id DESC");
									$prefix = $arr[0]['prefix'];
								}
																...
																<?php 
																	...
																	while($result = mysql_fetch_assoc($q)){
																		...
																		<tr>
																			...
																			<td class='text-center'><button onclick="clip_value('remove_grouping','<?php echo $result['id'];?>')" data-toggle="modal" data-target="#dell_grouping" class="btn btn-danger btn-sm"><i class='fa fa-times-circle'></i> Remove</button></td>
																		</tr>
																		<?php
	...
	<div class="modal fade" id="add" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                    ...
                    <div class='row'>
									<span id="unit_create_msgbox"></span>
										<div class='col-md-12'>
													...
													<div class="form-group">
														<label>Action</label>
														<button onclick=retrieve({'act':'create_grouping'}) class="btn btn-primary full-width">Create</button>
													</div>
										</div>
		...
		<div class="modal fade" id="dell_grouping" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				<div class="modal-header label-primary white">
					<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
					<h4 class="modal-title" >Remove Grouping (???)</h4>
				</div>
				<div class="modal-body">
					<div class="block">
						<span class="text-danger">
							<p><h4>Are you sure you want to remove this Grouping ?</h4></p>
							<p>Deleting this Grouping will result in the following effects :</p>
							<ul>
								<li>Loss of Data added against this Grouping.</li>
								<li>Loss of Stock added against this Grouping.</li>
							</ul>
				...
				<div class="modal-footer">
					<form method="POST">
						<button type="submit" id="remove_grouping" name="remove_grouping" type="button" class="btn btn-success btn-xs">Remove (???) </button>
