	...
	switch(arr['act']){
			...
			var item = parseFloat($.trim($("#item").val()));
				...
				if(retrieve({'act':'sale_row_verify'})){
							...
							if($('tr[item="'+item+'"]').length>0){
								...
								//$('#tbl tr[item="'+item+'"]').children('td').css('background-color','#EDB3A7');
		...
		break;
			...
			case "complete_sale":
				...
				var item = {'item':array};
				var aux = {'aux':{'discount':discount,'customer':customer,'aging':aging,'paid':paid,'company':company}};
				var data = {'item':item,'aux':aux};
			...
			if(amt>remaining){
				...
				$.ajax	
							(
									{
											type: "POST",
											url: "assets/ajax/proc.php?act=sale_add_paid&inv="+arr['inv']+"&paid="+amt,
		...
		case "up_sale_item_qty":
			...
			var price = $.trim($("#curr_price").val());
		
				$.ajax	
							(
									{
											type: "POST",
											url: "assets/ajax/proc.php?act=up_sale_item_qty"+"&item="+arr['item']+"&qty="+qty+"&price="+price,
