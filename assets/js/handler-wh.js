...
if(company!='' && name!='' && quantity!='' && volume!='' && unit!='' && price_per_unit!='')
					{
							var dataString = "act=create_item&company="+company+"&name="+name+"&quantity="+quantity+"&volume="+volume+"&unit="+unit+"&price_per_unit="+price_per_unit;
	...
	switch(arr['act']){
			...
			var item = parseFloat($.trim($("#item").val()));
				...
				if(retrieve({'act':'sale_row_verify'})){
							...
							if($('tr[item="'+item+'"]').length>0){
								...
								var details = jQuery.parseJSON(retrieve({'act':'simple_xhr','url':'assets/ajax/proc.php?act=sale_wh_item_qty_price&item='+item}));
		...
		break;
			...
			case "complete_sale":
				...
				var dataString = "assets/ajax/proc.php?act=create_sale_invoice_wh&data="+encodeURIComponent(JSON.stringify(data))
				$.ajax	
					(
						{
							type:"POST",
							...
							success:function(result)
							{
								...
								if(result['status']=="success"){
									switch(state){
											...
											case "credit":
												window.location = "sale_invoice_detail-wh?inv="+result['message'];
											break;
											
											case "cash":
												window.location = "sale_cash_invoice_detail-wh?inv="+result['message'];
											break;
									}
		...
		case "inv_sale_up_remarks":
			...
			var dta = retrieve({'act':'simple_xhr','url':'assets/ajax/proc.php?act=inv_sale_up_remarks_wh&inv='+arr['inv']+"&status="+status+"&remarks="+remarks});
		...
		case "reorder_sale_rows":
				...
				$.each(unique, function (index, value) {
					...
					var details = jQuery.parseJSON(retrieve({'act':'simple_xhr','url':'assets/ajax/proc.php?act=sale_wh_item_qty_price&item='+value}));
		...
		case "sale_rem_paid":
				...
				$.ajax	
							(
									{
											type: "POST",
											url: "assets/ajax/proc.php?act=sale_rem_paid_wh&paid="+arr['paid'],
						...
						}else
						{
							$.ajax	
										(
												{
														type: "POST",
														url: "assets/ajax/proc.php?act=sale_add_paid_wh&inv="+arr['inv']+"&paid="+amt+"&payment_type="+payment_type+"&desc="+desc,
						...
						}else
						{
							$.ajax	
										(
												{
														type: "POST",
														url: "assets/ajax/proc.php?act=sale_add_paid_wh&inv="+arr['inv']+"&paid="+amt+"&payment_type="+payment_type+"&issue_to="+issue_to+"&bank="+bank+"&c_number="+c_number+"&dated="+dated+"&desc="+desc,
			...
			var qty = $.trim($("#edit_curr_qty").val());
				...
				$.ajax	
							(
									{
											type: "POST",
											url: "assets/ajax/proc.php?act=edit_sale_item_qty"+"&item="+arr['item']+"&qty="+qty,
					...
					$("#notify_return").html("<div class='alert alert-danger'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'><i class='fa fa-times-circle'></i></button><strong> Error!</strong> Cannot exceed total quantity</div>");
						...
						$.ajax	
							(
									{
											type: "POST",
											url: "assets/ajax/proc.php?act=return_item_wh"+"&item="+arr['item']+"&qty="+qty+"&remarks="+remarks,
		...
		case "cheque_detail":
			...
			var url = 'assets/ajax/proc.php?act=cheque_detail_wh&inv='+arr['inv']+'&sale_paid_id='+arr['sale_paid_id'];
		...
		case "fetch_report":
			...
			if(company!="" && report_type!="" && from!="" && to!="" ){
				var url = "assets/ajax/proc.php?act=reports"+"&report_type="+report_type+"&from="+from+"&to="+to+"&company="+company;
				var dta = retrieve({'act':'simple_xhr','url':url});
				if(dta!=""){
