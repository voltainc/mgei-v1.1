...
$invdtls = $member->retrieve(['act'=>'sale_invoice_details','inv'=>trim($_REQUEST['inv']),'type'=>'cash']);
...
if(isset($_POST['upload'])){
	...
	$status = ($member->upload($_FILES['document'],"uploads/invoice/sale/cash",$invdtls['invoice_detail']['id']));
										...
										<div class="col-md-12">
                                            ...
                                            <ul class="ls-feed">
                                                <?php 
													...
													$docs = @$member->dirToArray("uploads/invoice/sale/cash/{$invdtls['invoice_detail']['id']}");
													if(!empty($docs))
													{
														foreach($docs as $val)
														{
												?>
														<li> 
															...
															<a href="javascript:void(0)" onclick="OpenInNewTab('<?php echo "uploads/invoice/sale/cash/{$invdtls['invoice_detail']['id']}/{$val}";?>')"><?php echo $val; ?></a>
															<a title='Remove (نزع)' class="text-danger" href="javascript:void(0)" data-toggle="modal" data-target="#unlink" onclick="clip_value('remove_document','<?php echo "uploads/invoice/sale/cash/{$invdtls['invoice_detail']['id']}/{$val}";?>')"><span class="date"><i class='fa fa-times-circle'></i></span></a>
														</li>
												<?php 
														}
													}else{
