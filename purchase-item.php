...
<?php echo $static->sidebar("inv-purchase-item");?>
					...
					<?php
                     ...
                     <?php 
								if(isset($val_company)){
									$q = mysql_query("select * from item_purchase where company='{$val_company['id']}' and created_by='{$_SESSION['clerk']['id']}' and status=true order by id DESC");
									$prefix = $val_company['prefix'];
								}else{
									$q = mysql_query("select * from item_purchase where company='{$arr[0]['id']}' and created_by='{$_SESSION['clerk']['id']}' and status=true order by id DESC");
									$prefix = $arr[0]['prefix'];
								}
																	...
																	while($result = mysql_fetch_assoc($q)){
																		...
																		<tr>
																			...
																			<td class='text-center'>
																				<!--
																					<button  data-toggle="modal" data-target="#edit_quantity" class="btn btn-xs btn-round btn-primary" onclick=retrieve({'act':'edit_purchase_item_qty','item':'<?php echo $result['id']?>','qty':'<?php echo $result['quantity']?>'})><i class="fa fa-pencil"></i></button>
																				-->
